...
model = dict(
    ...
    test_cfg=dict(
        pts_head=dict(
            use_rotate_nms=True,
            nms_across_levels=False,
            nms_thr=0.01, # not 0.01 only because is intermediate
            score_thr=0.05,
            min_bbox_size=0,
            nms_pre=1000,
            max_num=1000),
    ...
    train_cfg=dict(
        pts_head=dict(
            ...
            use_rotate_nms=True,
            nms_across_levels=False,
            nms_thr=0.5, # only because is intermediate
            score_thr=0.05,
            min_bbox_size=0,
            nms_pre=1000,
            max_num=1000),
...
optim_wrapper = dict(
    optimizer=dict(lr=1e-5, momentum=0.9, type='SGD', weight_decay=0.0001),
    type='OptimWrapper')
param_scheduler = [
    dict(begin=0, by_epoch=False, end=500, start_factor=0.001, type='LinearLR'),
    dict(type='ReduceOnPlateauLR', factor=0.1, patience=10, rule='greater', monitor='Kitti metric/pred_instances_3d/KITTI/Overall_3D_AP40_hard', min_value=1e-5),
]
